...
(* Please copy this file,rel_veceq.o,rel_veceq2.o to the root directory of
...
let veceq_mc = define_assert_from_elf "veceq_mc" "rel_veceq.o" [
...
let veceq2_mc = define_assert_from_elf "veceq2_mc" "rel_veceq2.o" [
...
let equiv_input_states = new_definition
  `!s1 s1' x.
    ...
    (read X1 s1 = x /\ read X1 s1' = x /\
     ?a. bignum_from_memory (x,2) s1 = a /\
         bignum_from_memory (x,2) s1' = a)`;;

let equiv_output_states = new_definition
  `!s1 s1'.
    (equiv_output_states:(armstate#armstate)->bool) (s1,s1') <=>
    (?a. read X20 s1 = a /\ read X21 s1' = a /\
     (?b. read X21 s1 = b /\ read X24 s1' = b /\
      (?c. read X22 s1 = c /\ read X19 s1' = c /\
       (?d. read X23 s1 = d /\ read X4 s1' = d))))`;;


(* Define the equivalence statement which is ensures2 predicate.
   Please look at the definition of mk_equiv_statement for full
