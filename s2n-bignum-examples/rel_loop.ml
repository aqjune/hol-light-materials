...
(* Please copy this file,rel_loop.o and rel_loop2.o to the root directory of
...
let loop_mc = define_assert_from_elf "loop_mc" "rel_loop.o" [
...
let loop2_mc = define_assert_from_elf "loop2_mc" "rel_loop2.o" [
...
let LOOP_EQUIV = prove(
  ...
  REPEAT CONJ_TAC THENL [
    ...
    ARM_STUTTER_LEFT_TAC LOOP_EXEC (1--3) THEN
    (* Symbolically execute the right program only. *)
    ARM_STUTTER_RIGHT_TAC LOOP2_EXEC (1--4) "'" THEN
    ...
    ARM_STUTTER_LEFT_TAC LOOP_EXEC (1--1) THEN
    ARM_STUTTER_RIGHT_TAC LOOP2_EXEC (1--1) "'" THEN
    REPEAT_N 2 ENSURES_FINAL_STATE'_TAC THEN
    ASM_REWRITE_TAC[WORD_ADD] THEN
